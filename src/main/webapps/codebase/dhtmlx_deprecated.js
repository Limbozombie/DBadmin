...
dhtmlXTreeObject.prototype.addPath = function (b, e, c, h) {
    ...
    while (a != g) {
        d.push({
            open: this._getOpenState(f),
            from: f.id,
            size: (g ? this._getIndex(g) : 0),
            to: (g ? g.id : null),
            style: "border-left:" + (c.width || 1) + "px " + (c.mode || "solid")
            + " " + (c.color || "red") + "; border-bottom:" + (c.width || 1)
            + "px " + (c.mode || "solid") + " " + (c.color || "red") + ";"
        });
        g = f;
        f = f.parentObject
    }
...
dhtmlXTreeObject.prototype.activatePaths = function (a) {
    ...
    this.attachEvent("onXLE", function (e) {
        var d = dhx4.ajax.xpath("//pathend", e);
        var h = dhx4.ajax.xpath("//pathstart", e);
        var g = {};
        for (var f = 0; f < h.length; f++) {
            g[h[f].getAttribute("id")] = h[f]
        }
        for (var f = 0; f < h.length; f++) {
            var c = d[f].parentNode;
            var j = g[d[f].getAttribute("id")];
            this.addPath(j.parentNode.getAttribute("id"), c.getAttribute("id"),
                {
                    color: j.getAttribute("color"),
                    mode: j.getAttribute("mode"),
                    width: j.getAttribute("width")
                }, j.getAttribute("id"))
        }
    });
    if (a) {
        ...
        if (this._idpull[0].childsCount) {
            this._halfHeight = Math.floor(this._idpull[0].childNodes[0].span.parentNode.offsetHeight
                / 2)
        }
    }
    if (!this._halfHeight) {
...
dhtmlXTreeObject.prototype._renderPath = function (g) {
    var k = this._idpull[g.path[g.path.length
    - 1].from].span.parentNode.parentNode;
    ...
    for (var b = g.path.length - 1; b > 0; b--) {
        ...
        h.style.cssText = "position:absolute; z-index:1; width:" + (_isIE ? 10
            : 8) + "px; height:" + (j - 9) + "px; left:" + e + "px; top:" + f
            + "px;" + a.style;
        f += j;
        e += 18;
...
dhtmlXTreeObject.prototype._renderState = function () {
    ...
    this._getItemByPos(b, this.itemHeight, a, null, false,
        this._renderItemSRND);
    ...
    if (!k.span) {
        ...
        for (var c = 1; c < f.length; c++) {
            ...
            var g = l.nodem ? this.itemHeight : (l.offsetHeight || parseInt(
                l.childNodes[1].firstChild.style.height));
            e -= g;
            if (e < 0) {
                if (e == -1) {
                    e++;
                ...
                d.style.height = (parseInt(d.style.height) - (g - Math.abs(e)
                    + this.itemHeight)) + "px";
                if (Math.abs(e) != g) {
...
dhtmlXTreeObject.prototype._singleTimeSRND = function () {
    ...
    window.dhtmlXTreeItemObject = function (g, c, d, b, e, f) {
        ...
        this.images = new Array(b.imageArray[0], b.imageArray[1],
            b.imageArray[2]);
    ...
    this.deleteChildItems = function (b) {
        if (this.rootId == b) {
            ...
            this.allTree.insertBefore(this.htmlNode.htmlNode,
                this.selectionBar);
            if (_isFF) {
...
dhtmlXGridObject.prototype.makePivot = function (j, a) {
    ...
    if (a.column_list) {
        ...
        for (var g = 0; g < this.hdr.rows[1].cells.length; g++) {
            this._column_list.push(
                this.hdr.rows[1].cells[g][_isIE ? "innerText" : "textContent"])
        }
    }
    var h = this;
...
dhtmlXGridObject.prototype._renderPivot2 = function () {
    if (!(this._pivotS.x && this._pivotS.y && this._pivotS.value
        && this._pivotS.action)) {
        return
    }
    ...
    for (var c = 0; c < a.length; c++) {
        for (var b = 0; b < e.length; b++) {
            this._pgrid.cells2(b, c + 1).setValue(Math.round(d(this._pivotS.x,
                this._pivotS.y, this._pivotS.value, a[c], e[b], this._pData)
                * 100) / 100)
        }
    }
};
    ...
    this.detach = function () {
        ...
        if (this.list.parentNode != null) {
            if ((this.obj.value._dhx_trim() != this.text) || (this._byClick)) {
                ...
                if (!this._byClick) {
                    this.combo.values[this.combo.keys._dhx_find(
                        b)] = this.obj.value
                }
                this.setValue(b)
            } else {
...
function eXcell_wbut(a) {
    ...
    this.edit = function () {
        ...
        if (_isIE) {
            e.style.height = (this.cell.offsetHeight - (this.grid.multiLine ? 5
                : 4)) + "px";
...
function eXcell_passw(a) {
    ...
    this.edit = function () {
        ...
        this.obj.style.height = (this.cell.offsetHeight - (_isIE ? 6 : 4))
            + "px";
        this.obj.className = "dhx_combo_edit";
...
function eXcell_num(a) {
    ...
    this.getValue = function () {
        if ((this.cell.firstChild) && (this.cell.firstChild.tagName
            == "TEXTAREA")) {
...
function eXcell_liveedit(a) {
    ...
    this.onFocus = function () {
        var b = this.grid.callEvent("onEditCell",
    ...
    this.onBlur = function () {
        var b = this.grid.callEvent("onEditCell",
            [2, this.cell.parentNode.idd, this.cell._cellIndex]);
        this.detach()
    };
    this.onChange = function () {
        var b = this.grid.callEvent("onCellChanged",
            [this.cell.parentNode.idd, this.cell._cellIndex,
                this.cell.inputObj.value]);
        this.detach()
    }
}
...
function eXcell_limit(a) {
    ...
    this.edit = function () {
        this.cell.atag = ((!this.grid.multiLine) && (_isKHTML || _isMacOS
            || _isFF)) ? "INPUT" : "TEXTAREA";
        this.val = this.getValue();
        this.obj = document.createElement(this.cell.atag);
        this.obj.style.height = (this.cell.offsetHeight - (_isIE ? 6 : 4))
            + "px";
    ...
    this.getValue = function () {
        if ((this.cell.firstChild) && ((this.cell.atag)
            && (this.cell.firstChild.tagName == this.cell.atag))) {
            return this.cell.firstChild.value
        } else {
...
if (window.dhtmlxHierarchy) {
    ...
    eXcell_tree_property.prototype.setValue = function (a) {
        if (this.cell.parentNode.imgTag) {
            return this.setLabel(a)
        }
        if ((this.grid._tgc.iconTree == null) || (this.grid._tgc.iconTree
            != this.grid.iconTree)) {
            var d = {};
            d.imst = "<img src='" + this.grid.iconTree;
            d.imsti = "<img src='" + (this.grid.iconURL || this.grid.iconTree);
            d.imact = "' align='absmiddle'  onclick='this." + (_isKHTML ? ""
                : "parentNode.")
                + "parentNode.parentNode.parentNode.parentNode.grid.doExpand(this);event.cancelBubble=true;' class='property_image'>";
            d.plus = d.imst + "plus.gif" + d.imact;
            d.minus = d.imst + "minus.gif" + d.imact;
            d.blank = d.imst + "blank.gif" + d.imact;
            d.start = "<div style=' overflow:hidden; white-space : nowrap; height:"
                + (_isIE ? 20 : 19) + "px;'>";
            d.itemim = "<span " + (_isFF ? "style='position:relative; top:2px;'"
                : "") + "id='nodeval'>";
        ...
        if (this.grid.kidsXmlFile || this.grid._slowParse) {
            f.has_kids = (f.has_kids || (this.cell.parentNode._attrs.xmlkids
                && (f.state != "minus")));
        ...
        if (f.childs.length) {
            this.grid.getRowById(
                this.cell.parentNode.idd)._attrs["class"] = " dhx_parent_row ";
...
function dhtmlXPropertyGrid(a) {
    ...
    b.attachEvent("onEditCell", function (f, j, h, e, g) {
        if (f == 1 && this.editor && this.editor.obj
...
dhtmlXTabBar.prototype.setHrefMode = function (a) {
    if (a == "iframes-on-demand" || a == "ajax-html") {
        this.conf.url_demand = true
    }
    this.conf.href_mode = a
};
dhtmlXTabBar.prototype.setContentHref = function (b, a) {
    if (this.conf.href_mode == null) {
        this.conf.href_mode = "iframe"
    }
    switch (this.conf.href_mode) {
        ...
        case"iframes-on-demand":
            this.conf.urls[b] = {href: a, ajax: false};
